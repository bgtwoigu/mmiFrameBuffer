...
static void GpsAutoSetDateTime(char isInvalid)
{
	...
	if((gyear < 2015)||(gyear == 2080)||(!gmonth)||(!gday))
		return;
	GetDateTime(&oldtime);
	if(isInvalid && (oldtime.nYear > gyear))
		return;
...
static void YxAppGpsRestartTimer(void)
{
	...
	StartTimer(POC_IND_TIMER,yxGpsParam.gpsTimerVal*1000,GpsGetDataTimeOut);
#if(YX_IS_TEST_VERSION!=0)
...
static void YxAppGpsPowerOffTimer(void)
{
	YXAppGpsPowerCtrl(0);
	StopTimer(POC_IND_TIMER);
	StartTimer(POC_IND_TIMER,2000,YxAppGpsRestartTimer);
}
#endif
#endif
...
void YxAppGpsControl(char operation)
{
	...
	{
#ifdef WIN32
		StartTimer(POC_IND_TIMER,1000,GpsGetDataTimeOut);
		return;
#endif
#if(YX_IS_TEST_VERSION!=0)
	...
	//	YxAppDisableSleep();
		...
		if(yxGpsParam.yxAgpsDatLen>20)
		{
			...
			StartTimer(POC_IND_TIMER,yxGpsParam.gpsTimerVal*1000,GpsGetDataTimeOut);
#else
			StartTimer(POC_IND_TIMER,1500,YxAppGpsPowerOffTimer);
#endif
		}
		else
		{
